...
fn download_to(url: &str, path: &Path) -> Result<(), String> {
    if path.exists() && fs::metadata(path).map_err(|e| e.to_string())?.len() > 0 { return Ok(()); }
...
fn safe_extract<R: Read>(mut zip: zip::read::ZipFile<'_, R>, root: &Path) -> Result<(), String> {
...
fn download_library_list(libs: &[Value], libraries: &Path) -> Result<(), String> {
    for lib in libs {
        if !allowed(lib) { continue; }
        if let Some(p)=artifact_path(lib,libraries) {
            if let Some(u)=lib.get("downloads").and_then(|x|x.get("artifact")).and_then(|x|x.get("url")).and_then(Value::as_str) { download_to(u,&p)?; }
        } else if let Some(name)=lib.get("name").and_then(Value::as_str) {
            if let Some(base)=lib.get("url").and_then(Value::as_str) {
                if let Some(path)=maven_path(name) { download_to(&format!("{}{}",base,path),&libraries.join(&path))?; }
            }
        }
    }
    Ok(())
...
#[tauri::command]
fn launcher_info()->Value{json!({"name":"Sakura Launcher","version":"0.4.0","status":"minecraft-core"})}
...
fn list_instance_files(app:tauri::AppHandle,id:String)->Result<Value,String>{
    ...
    Ok(json!({"mods":list,"path":dir.to_string_lossy()}))
}
...
fn install_minecraft(app:tauri::AppHandle,id:String,version:String,loader:String)->Result<Value,String>{
    ...
    download_to(client.get("url").and_then(Value::as_str).ok_or("Нет URL client.jar")?,&vdir.join(format!("{}.jar",version)))?;
    ...
    if let Some(arr)=meta.get("libraries").and_then(Value::as_array){
        for lib in arr { if !allowed(lib){continue;} if let Some(u)=lib.get("downloads").and_then(|x|x.get("classifiers")).and_then(|x|x.get("natives-windows")) .and_then(|x|x.get("url")).and_then(Value::as_str){native_urls.push(u.to_string());} }
    }
    let natives=game.join("natives"); fs::create_dir_all(&natives).map_err(|e|e.to_string())?;
    for u in native_urls { let bytes=http_get(&u)?; let mut z=zip::ZipArchive::new(Cursor::new(bytes)).map_err(|e|e.to_string())?; for i in 0..z.len(){ let f=z.by_index(i).map_err(|e|e.to_string())?; if f.name().starts_with("META-INF/"){continue;} safe_extract(f,&natives)?; } }
    if let Some(ai)=meta.get("assetIndex"){
        let aid=ai.get("id").and_then(Value::as_str).unwrap_or("legacy");
        let au=ai.get("url").and_then(Value::as_str).ok_or("Нет URL asset index")?;
        let idx=game.join("assets").join("indexes"); fs::create_dir_all(&idx).map_err(|e|e.to_string())?;
        let ip=idx.join(format!("{}.json",aid)); download_to(au,&ip)?;
        let data:Value = serde_json::from_slice(&fs::read(&ip).map_err(|e|e.to_string())?).map_err(|e|e.to_string())?;
        if let Some(objects)=data.get("objects").and_then(Value::as_object){
            for (_,o) in objects { if let Some(h)=o.get("hash").and_then(Value::as_str){ if h.len()<2 {continue;} let p=game.join("assets").join("objects").join(&h[0..2]).join(h); let u=format!("https://resources.download.minecraft.net/{}/{}",&h[0..2],h); download_to(&u,&p)?; } }
    }
    ...
    if loader.eq_ignore_ascii_case("fabric") {
        ...
        if let Some(arr)=profile.get("libraries").and_then(Value::as_array){download_library_list(arr,&libs)?;}
        fs::write(vdir.join("fabric.json"),serde_json::to_vec_pretty(&profile).map_err(|e|e.to_string())?).map_err(|e|e.to_string())?;
        loader_info=json!({"name":"Fabric","version":lv});
    }
    Ok(json!({"installed":true,"instance":id,"version":version,"loader":loader,"loaderInfo":loader_info,"game_dir":game.to_string_lossy()}))
}
...
fn launch_instance(app:tauri::AppHandle,id:String,version:String,username:String,loader:String,java_path:Option<String>)->Result<(),String>{
    ...
    vars.insert("launcher_version".into(),"0.4.0".into());
    let mut cp=Vec::new();
